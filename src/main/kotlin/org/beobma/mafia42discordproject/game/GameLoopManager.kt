...
object GameLoopManager {
    ...
    suspend fun startNightPhase(game: Game) {
        ...
        game.sendMainChannerMessage(
            msg = "밤이 되었습니다.",
            imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1483978042673070342/43e6c3860a090af9.png?ex=69be8800&is=69bd3680&hm=1dabf5630544f8f8766c7abbb0793a48e3a11e1364a31d1e4e439fff70539e25&"
        )
    ...
    suspend fun startDayPhase(
        ...
        game.sendMainChannerMessage(
            msg = dawnPresentation.message,
            imageUrl = dawnPresentation.imageUrl
        )
        delay(3_000L)

        game.sendMainChannerMessage(
            msg = "날이 밝았습니다.",
            imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1483981622096429247/7aace941ae58a6cc.png?ex=69bc9115&is=69bb3f95&hm=fc7255667bb001a0f730a3e42d5d729c8584db33095699bcb02fc4ea4295a613&"
        )
    ...
    suspend fun resolveVotePhase(game: Game): PlayerData? {
        ...
        if (invalidVoteCount >= maxVotes || maxVotes == 0) {
            game.sendMainChannerMessage(
                msg = "처형될 대상을 고르지 못했습니다.",
                imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1484594233653465122/K5WjViOFIiajx3YUfctCF-wkTWwg-DnerBQ09EXEd5-Jxz6Yy0vAmAuM5XDOMIWqHpYOXk85dCobA6CkwzPxOILsPNTbKJgtpYa1DtnVqhceybFNoLK5kdEtPJr6x7rCpn5F3Au_wTeTK0zWtRNArQ.webp?ex=69becb9f&is=69bd7a1f&hm=95cc33354d29bf53d2a74db6ca5ac622b88ef11bfe5b9e419f6e7b38a6f2a8b4&"
            )
            return null
        }
    ...
    suspend fun startDefensePhase(game: Game, target: PlayerData) {
        val mainChannel = game.mainChannel ?: return
        game.sendMainChannerMessage(
            msg = "${target.member.effectiveName}의 최후의 변론",
            imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1484595217796567092/b1bb8f82a19e45e3.png?ex=69becc8a&is=69bd7b0a&hm=0facb3df92275cbd87534a5c337cb4c774643de1c0ec93529a105c1573f30f35&"
        )
    ...
    suspend fun resolveExecutionPhase(game: Game, target: PlayerData) {
        ...
        if (!executionEvent.isApproved) {
            game.sendMainChannerMessage(
                msg = "${target.member.effectiveName}님의 처형이 부결되었습니다.",
                imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1484594233653465122/K5WjViOFIiajx3YUfctCF-wkTWwg-DnerBQ09EXEd5-Jxz6Yy0vAmAuM5XDOMIWqHpYOXk85dCobA6CkwzPxOILsPNTbKJgtpYa1DtnVqhceybFNoLK5kdEtPJr6x7rCpn5F3Au_wTeTK0zWtRNArQ.webp?ex=69becb9f&is=69bd7a1f&hm=95cc33354d29bf53d2a74db6ca5ac622b88ef11bfe5b9e419f6e7b38a6f2a8b4&"
            )
            return
        }
        ...
        game.nightEvents.clear()
        game.sendMainChannerMessage(
            msg = "${target.member.effectiveName}님이 투표로 처형당하였습니다.",
            imageUrl = "https://cdn.discordapp.com/attachments/1483977619258212392/1484594233288691895/22SIfKIG4sgmfsgKpScS00MYCCNg70dZoYW9wB3zjuIlnN7d56sqkmFViOFPYrPnPJixJ-BEj5f_mVUp2wcYAzYpHKjyZDuoQyzfp3efnGqc1UYKkMLrk0w5QxCV5tlorhBipi2-c69B7eSYhppyIA.webp?ex=69becb9f&is=69bd7a1f&hm=0b3d5473bbaebb91f2335ef3d07cf315043fde1889930328ea4211c486e792df&"
        )
    }
