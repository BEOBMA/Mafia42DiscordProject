...
class Trick : ActiveAbility, JobUniqueAbility {
    ...
    override val description: String =
        "낮 또는 투표 시간에 생존한 다른 플레이어를 미리 골라 자신이 최후의 반론에 오를 경우 그 플레이어를 대신 최후의 반론에 올린다. 바꿔치기에 성공한 경우 이 능력을 다시는 사용할 수 없으며, 자신의 정체가 모두에게 공개된다."
    ...
    override fun activate(game: Game, caster: PlayerData, target: PlayerData?): AbilityResult {
        if (game.currentPhase !in setOf(GamePhase.DAY, GamePhase.VOTE) || game.defenseTargetId != null) {
            return AbilityResult(false, "트릭은 낮 또는 투표 시간에만 사용할 수 있습니다.")
        }
